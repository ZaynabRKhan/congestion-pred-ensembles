...
def multi_process_score(out_name=None, threshold=0.0, label_path=None, save_path=None):
    ...
    preds = []
    for root, dirs, files in os.walk(os.path.join(save_path, 'test_result')):
        for file in files:
            if file.endswith('.npy'):
                preds.append(os.path.relpath(
                    os.path.join(root, file),
                    os.path.join(save_path, 'test_result')
                ))
    ...
    src = os.path.join(temp_path, f'{out_name}')
    dst = os.path.join(os.getcwd(), save_path, f'{out_name}')
